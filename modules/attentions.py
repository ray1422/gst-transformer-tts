...
class BahdanauAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs, seq_lengths=None, training=None):
        """
    ...
    @staticmethod
    def initial_alignment_fn(batch_size, key_time, dtype):
...
class BahdanauMonotonicAttention(tf.keras.layers.Layer):
    ...
    def _apply_scores(self, score, value, previous_alignment, seq_lengths):
        '''
        ...
        alignment = self._monotonic_probability_fn(score, previous_alignment, seq_lengths)  # [Batch_size, 1, T_v]
        masks = tf.expand_dims(tf.sequence_mask(seq_lengths, alignment.shape[2], dtype=tf.float32), axis=1)
        alignment *= masks  # padding mask
        context = tf.matmul(alignment, value)  # [Batch_size, 1, Att_dim]

        return tf.squeeze(context, axis=1), tf.squeeze(alignment, axis=1)

    def _monotonic_probability_fn(self, score, previous_alignment, seq_lengths):
...
class StepwiseMonotonicAttention(BahdanauMonotonicAttention):
    ...
    def _monotonic_probability_fn(self, score, previous_alignment, seq_length=None):
        """
        ...
        if seq_length is not None:
            masks = tf.expand_dims(tf.sequence_mask(seq_length, score.shape[2], dtype=tf.bool), axis=-1)
            score = tf.where(masks, score, tf.ones_like(score, dtype=tf.float32) * -1000)
